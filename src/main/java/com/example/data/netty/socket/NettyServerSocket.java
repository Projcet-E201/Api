...
public class NettyServerSocket {
	...
	public void start() {
		try {
			...
			// sync -> 닫힐때 발생하는 ChannelFuture 객체의 sync 메서드 호출 채널이 닫힐 때 까지 현재 스레드 블로킹 (서버 소켓 채널이 닫히기 전까지 실행을 중단, 서버가 계속 실행되며 들어오는 연결처리)
			// channel -> 서버 소켓채널이 닫힌 후 해당 채널 객체를 다기 사져옴
			serverChannelFuture.channel().closeFuture().addListener((ChannelFuture future) -> {
